...
uint8_t ee_ReadBytes(uint8_t *_pReadBuf, uint32_t _usAddress, uint16_t _usSize)
{
    ...
    else i2c_SendByte(EE_DEV_ADDR | I2C_WR | 0x02);
    ...
    else i2c_SendByte(EE_DEV_ADDR | I2C_RD | 0x02);
...
uint8_t ee_WriteOneBytes(uint8_t DataWrite, uint32_t usAddr)
{
    ...
    {
        i2c_SendByte(usAddr >> 8);//高8位
        if (i2c_WaitAck() != 0)
        {
            goto cmd_fail;	/* EEPROM器件无应答 */
        }
        
        i2c_SendByte(usAddr & 0xFF);//低8位
        if (i2c_WaitAck() != 0)
        {
            goto cmd_fail;	/* EEPROM器件无应答 */
        }
